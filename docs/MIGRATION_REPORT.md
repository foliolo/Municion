...
- [x] **Fase 7 — Plataforma** (núcleo completo en ambas plataformas; follow-ups iOS/permisos documentados)
  ...
  - [x] RevenueCat remove-ads con `purchases-kmp` 3.0.5 (**API común, sin expect/actual ni bridge Swift**; dep iOS vía Gradle): `RevenueCatRemoveAdsManager` (configure + entitlement `ad_free` vía `awaitCustomerInfo`/`awaitPurchase`/`awaitRestore`/`awaitLogIn`), key por plataforma vía `revenueCatApiKey()` expect/actual (Play debug→test, Play release→prod, iOS→App Store), binding Koin, UI compra/restaurar en AccountSettings. Degrada a no-op si la key está vacía. *Spike iOS confirmado: `:shared:linkDebugFrameworkIosSimulatorArm64` enlaza con llamadas reales a `Purchases` — la dep nativa iOS la trae Gradle, sin SPM/cocoapods.* Reconcile con `AppPurchase`/`ads_removed` no necesario v1 (RevenueCat persiste el entitlement offline).
  - [x] Calendario: interfaz común `CalendarManager` + `AndroidCalendarManager` (CalendarContract, 2 eventos: día de caducidad + 30 días antes) + `IosCalendarManager` (**EventKit nativo Kotlin/Native, sin bridge Swift** — `requestAccessToEntityType`/`saveEvent`/`removeEvent`/predicados; cinterop verificado al enlazar el framework iOS). Bindeado en `platformModule`; alta crea eventos (best-effort) y borrado los elimina. Petición de permiso `WRITE_CALENDAR` resuelta con `rememberPermissionRequester` (aviso en el formulario de Licencia); `NSCalendarsUsageDescription` ya en Info.plist iOS.
  - [x] FCM **Android**: `MyFirebaseMessagingService` (SDK nativo Firebase Messaging en `:androidApp`, sin módulo GitLive) + canal de notificación + registro en manifest; muestra notificaciones de pushes (consola Firebase). Petición runtime de `POST_NOTIFICATIONS` (Android 13+) resuelta con `rememberPermissionRequester` (se pide al entrar autenticado). *iOS push (APNs + coordinator Swift + clave APNs en consola) documentado como trabajo Xcode (§4.A.4/§4.F).*
  - **Verificado:** Android ✅ · iOS link ✅
