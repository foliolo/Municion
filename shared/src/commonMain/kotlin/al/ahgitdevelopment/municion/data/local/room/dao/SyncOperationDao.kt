...
interface SyncOperationDao {
    ...
    /**
     * Coalesces consecutive UPSERTs for the same entity: replaces a pending UPSERT's
     * payload instead of inserting a new row. DELETE always inserts a new row.
     */
    @Transaction
    ...
    @Query(
        """
        UPDATE sync_outbox
        SET status = 'SYNCED', last_attempt_at = :now, last_error = NULL
        WHERE id = :id
        """,
    )
    ...
    @Query(
        """
        UPDATE sync_outbox
        SET status = 'PENDING', retry_count = retry_count + 1, last_attempt_at = :now, last_error = :error
        WHERE id = :id
        """,
    )
    ...
    @Query(
        """
        UPDATE sync_outbox
        SET status = 'FAILED', last_attempt_at = :now, last_error = :error
        WHERE id = :id
        """,
    )
    ...
    /** Resets rows stuck IN_FLIGHT back to PENDING (called at startup after a killed drain). */
    @Query(
        """
        UPDATE sync_outbox
        SET status = 'PENDING', last_error = 'reset_in_flight_at_startup'
    ...
    @Query(
        """
        UPDATE sync_outbox
        SET status = 'PENDING', retry_count = 0, last_attempt_at = NULL, last_error = NULL
        WHERE status = 'FAILED'
        """,
    )
