...
class CompraRepository(
    ...
    suspend fun saveCompra(compra: Compra, userId: String? = null): Result<Long> = runCatching {
    ...
    suspend fun updateCompra(compra: Compra, userId: String? = null): Result<Unit> = runCatching {
        val stamped = compra.copy(
            syncId = compra.syncId.ifBlank { SyncIdGenerator.newSyncId() },
            updatedAt = nowMillis(),
        )
    ...
    suspend fun deleteCompra(compra: Compra, userId: String? = null): Result<Unit> = runCatching {
        val now = nowMillis()
