    ...
    companion object {
        ...
        val MIGRATION_23_24 = object : Migration(23, 24) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                try {
                    ...
                    // ============================================================
                    android.util.Log.i("MunicionDatabase", "STEP 3: Migrating data from old tables...")

                    // Copiar GUIAS (directo, sin cambios)
                    val guiasCount = database.compileStatement("SELECT COUNT(*) FROM guias_old").simpleQueryForLong()
                    database.execSQL("""
                    ...
                    val comprasCount = database.compileStatement("SELECT COUNT(*) FROM compras_old").simpleQueryForLong()
                    database.execSQL("""
                        ...
                        FROM compras_old
                    """.trimIndent())
                    android.util.Log.i("MunicionDatabase", "✓ Migrated $comprasCount compras (peso TEXT→INTEGER)")

                    // Copiar LICENCIAS (directo)
                    val licenciasCount = database.compileStatement("SELECT COUNT(*) FROM licencias_old").simpleQueryForLong()
                    database.execSQL("""
                    ...
                    val tiradasCount = database.compileStatement("SELECT COUNT(*) FROM tiradas_old").simpleQueryForLong()
                    database.execSQL("""
                    ...
                    val finalGuias = database.compileStatement("SELECT COUNT(*) FROM guias").simpleQueryForLong()
                    val finalCompras = database.compileStatement("SELECT COUNT(*) FROM compras").simpleQueryForLong()
                    val finalLicencias = database.compileStatement("SELECT COUNT(*) FROM licencias").simpleQueryForLong()
                    val finalTiradas = database.compileStatement("SELECT COUNT(*) FROM tiradas").simpleQueryForLong()

                    android.util.Log.i("MunicionDatabase", "========================================")
                    android.util.Log.i("MunicionDatabase", "MIGRATION v23→v24 COMPLETED SUCCESSFULLY")
                    ...
                    android.util.Log.e("MunicionDatabase", "========================================")
        ...
        fun create(context: Context): MunicionDatabase {
            return Room.databaseBuilder(
                ...
                .fallbackToDestructiveMigration()  // Solo como último recurso
                .build()
        }
    }
}
