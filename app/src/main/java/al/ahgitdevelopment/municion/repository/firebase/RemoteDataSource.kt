...
import al.ahgitdevelopment.municion.repository.database.KEY_IMAGE
...
class RemoteDataSource @Inject internal constructor(
    ...
    override suspend fun removeAllLicenses() {
        ...
        auth.currentUser?.let { userId ->
            database.reference.child(ROOT).child(userId.uid).child(LICENSES).ref.removeValue()
        }
    }
    ...
    override suspend fun removeAllProperties() {
        ...
        auth.currentUser?.let { userId ->
            database.reference.child(ROOT).child(userId.uid).child(PROPERTIES).ref.removeValue()
        }
    }
    ...
    override suspend fun removeAllPurchases() {
        ...
        auth.currentUser?.let { userId ->
            database.reference.child(ROOT).child(userId.uid).child(PURCHASES).ref.removeValue()
        }
    }
    ...
    override suspend fun removeAllCompetitions() {
        ...
        auth.currentUser?.let { userId ->
            database.reference.child(ROOT).child(userId.uid).child(COMPETITIONS).ref.removeValue()
        }
    }

    override fun savePropertyImageItem(itemId: String, imageUrl: String) {
        Timber.v("Saved property image for item $itemId.\nUrl: $imageUrl")
        auth.currentUser?.let { userId ->
            database.reference.child(ROOT).child(userId.uid).child(PROPERTIES)
                .orderByChild(KEY_ID)
                .equalTo(itemId)
    ...
    override fun savePurchaseImageItem(itemId: String, imageUrl: String) {
        Timber.v("Saved purchase image for item $itemId.\nUrl: $imageUrl")
        auth.currentUser?.let { userId ->
            database.reference.child(ROOT).child(userId.uid).child(PURCHASES)
                .orderByChild(KEY_ID)
                .equalTo(itemId)
